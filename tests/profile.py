...
def show(f1, f2):
    ...
    for k,v in sorted(p1.stats.items(), key=lambda x: x[1][1]):# x[1][3] / x[1][0]):
...
if __name__=='__main__':
    ...
    sys.exit(0)
    if sys.argv[1] == 'p':
        cProfile.run('make_profile()', sys.argv[2])
        sys.exit(0)
    f1 = sys.argv[1]
